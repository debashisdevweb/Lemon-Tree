...
export function Brands() {
  return (
    ...
    >
      <div className="max-w-shell gap-xl mx-auto grid items-start lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        <Reveal>
        ...
        <div>
          ...
          <Reveal delay={260}>
            <ul className="mt-gap-heading bg-forest/18 grid list-none grid-cols-1 gap-px p-0 sm:grid-cols-2 xl:grid-cols-3">
              {BRANDS.tiles.map((tile) => (
                <li key={tile.name} className="bg-paper px-pad-tile py-pad-tile">
                  <span className="font-display text-h4 text-forest block leading-[1.1]">
                    {tile.name}
                  </span>
                  <span className="text-meta text-muted mt-label block">{tile.tier}</span>
                </li>
              ))}
            </ul>
          </Reveal>
